...
export default function IVIAufbietenDialog({ patient, onClose, onAufgeboten, arztName }: { patient: Patient; onClose: () => void; onAufgeboten?: () => void; arztName?: string }) {
  ...
  return (
    // Nicht-modal: kein abdunkelnder Backdrop, Klicks daneben gehen an die
    // App/Liris durch — das Fenster bleibt offen, während in Liris gearbeitet
    // wird, und behält seine (gezogene) Position (Nutzerwunsch 2026-08-01).
    <div className="fixed inset-0 z-[70] pointer-events-none flex items-center justify-center p-4">
      <div className="pointer-events-auto bg-white rounded-2xl shadow-2xl ring-1 ring-black/10 w-full max-w-lg max-h-[90vh] overflow-auto p-5" style={dragStyle} onClick={e => e.stopPropagation()}>
